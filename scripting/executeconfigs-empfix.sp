...
public Plugin:myinfo =
{
  ...
  author      = "Tsunami to v1.0, volt to v1.1, FirEXE to v1.2.2, Franug to v1.2.3, Neoony to 1.2.4",
...
{
	CreateConVar("sm_executeconfigs_version", PL_VERSION, "Execute configs on certain events.", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
	...
	else if(StrEqual(sGameDir, "empires"))
	{
		HookEvent("switching_to_new_map", 	Event_GameStart,  EventHookMode_PostNoCopy);
		HookEvent("commander_elected_player",   Event_RoundStart, EventHookMode_PostNoCopy);
	}
	else
